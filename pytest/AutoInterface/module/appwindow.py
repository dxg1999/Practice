...
class App_Window_test():
    ...
    def sw_RepeatStart_test(self):
        """
        小窗mini模式，再次进入小窗
        """
        logger.info("重复进入小窗，验证小窗模式的大小和位置")
        device.shell("input keyevent 187")
        device.implicitly_wait(3)
        device(resourceId="com.android.launcher3:id/name_tv", text="日历").click()
        time.sleep(3)
