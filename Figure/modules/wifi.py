...
class Wifi(unittest.TestCase):
    ...
    def wifi_connect_test(self,wifi_name):
        ...
        time.sleep(1)
        device.xpath(f'//*[@text="{wifi_name}"]').click()
        # 需切换为源生英文键盘
        time.sleep(2)
        logger.info("输入wifi密钥")
        os.system('adb shell input text 11112222')
        device.implicitly_wait(3)
        ...
        wlan_info = os.popen("adb shell ifconfig wlan0").read()
        logger.info(f"wifi的信息如下：\n{wlan_info}")
    ...
    def reconnect_wifi_test(self):
        ...
        if device(resourceId="android:id/summary", text="已连接").exists:
            ...
            error_info = logger.info("Wifi重连失败")
            Failed.failed_test(self, error_info)
